...
public sealed class AgentBindingForm : Form
{
    ...
    private readonly TextBox _code = new() { MaxLength = 6, Width = 180, Font = new Font("Segoe UI", 18, FontStyle.Bold), TextAlign = HorizontalAlignment.Center };
    ...
    {
        Text = "PC Rental · 手动绑定设备"; Width = 520; Height = 360; StartPosition = FormStartPosition.CenterScreen; FormBorderStyle = FormBorderStyle.FixedDialog; MaximizeBox = false; MinimizeBox = false; BackColor = Navy; ForeColor = Color.White; Font = new Font("Segoe UI", 10);
        var title = new Label { Text = "绑定 Windows 设备", AutoSize = true, Font = new Font("Segoe UI", 22, FontStyle.Bold), ForeColor = Mint, Location = new Point(34, 28) };
    ...
    private void Bind()
    {
        ...
        try
        {
            ...
            try { Process.Start(new ProcessStartInfo("sc.exe", "stop RentDeviceAgent") { Verb = "runas", UseShellExecute = true }); Thread.Sleep(1200); Process.Start(new ProcessStartInfo("sc.exe", "start RentDeviceAgent") { Verb = "runas", UseShellExecute = true }); } catch { _message.Text = "访问码已保存，请以管理员身份重启 RentDeviceAgent 服务。"; return; }
            Close();
        }
