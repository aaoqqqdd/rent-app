...
public sealed class AgentWorker : BackgroundService
{
    ...
    {
        if (string.IsNullOrWhiteSpace(_options.ApiBaseUrl) || string.IsNullOrWhiteSpace(_options.SerialNumber) || string.IsNullOrWhiteSpace(_options.SetupCode))
            throw new InvalidOperationException("ApiBaseUrl, SerialNumber and SetupCode are required");
    ...
    private void LoadState()
    {
        ...
        try { _token = JsonSerializer.Deserialize<State>(File.ReadAllText(_statePath))?.Token; } catch { _token = null; }
    }

    private void SaveState() => File.WriteAllText(_statePath, JsonSerializer.Serialize(new State(_token)));
    ...
    private sealed record RegisterResponse(bool Ok, string DeviceId, string Token);
